...
def convert(
    ...
    #### MVN
    for mod_sub_op, mod_sub_op_output, mod_div_op, mod_div_op_input in zip(mod_sub_ops.keys(), mod_sub_ops.values(),
                                                                           mod_div_ops.keys(), mod_div_ops.values()):
    ...
    #### Less
    for mod_add_op, mod_add_op_output, mod_less_op, mod_less_op_input in zip(mod_add_ops.keys(), mod_add_ops.values(),
                                                                             mod_less_ops.keys(),
                                                                             mod_less_ops.values()):
        # Finding outputs for connection
        mod_output = None
    ...
    #### Grater
    for mod_where_op, mod_where_op_output, mod_greater_op, mod_greater_op_input in zip(mod_where_ops.keys(),
                                                                                       mod_where_ops.values(),
                                                                                       mod_greater_ops.keys(),
                                                                                       mod_greater_ops.values()):
        # Finding outputs for connection
        mod_output = None
        for graph_node in graph.nodes:
    ...
    #### Expand
    for mod_unsqueeze_op, mod_unsqueeze_op_output, mod_expand_op, mod_expand_op_input, mod_gatherelements_op, mod_gatherelements_op_input in zip(
            mod_unsqueeze_ops.keys(), mod_unsqueeze_ops.values(), mod_expand_ops.keys(), mod_expand_ops.values(),
            mod_gatherelements_ops.keys(), mod_gatherelements_ops.values()):
        # Finding outputs for connection
        mod_output = None
        for graph_node in graph.nodes:
