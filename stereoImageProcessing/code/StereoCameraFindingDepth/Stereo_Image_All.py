...
left_eye = path + 'LeftImages/left_image_00' + '.jpg'
right_eye = path + 'RightImages/right_image_00' + '.jpg'
...
# get centre, top left and bottom right of boxes
...
tracks = scipy.optimize.linear_sum_assignment(cost)
h_d = [[np.array(weights.meta["categories"])[lbls[0]][i], np.array(weights.meta["categories"])[lbls[1]][j]] for i, j in
       zip(*tracks)]
...
for i, imgi in enumerate(imgs):
    ...
    annotate_class(img, deti[list(tracks[i])], cat_dist)
    cv2.imshow('LeftDepth', img)
