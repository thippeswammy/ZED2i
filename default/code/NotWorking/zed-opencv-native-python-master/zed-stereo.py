...
parser.add_argument("-fix", "--correct_focal_length", action='store_true',
                    help="correct for error in VGA factory supplied focal lengths for earlier production ZED cameras")
parser.add_argument("-fill", "--fill_missing_disparity", action='store_true',
...
parser.add_argument("-t", "--showcentredepth", action='store_true',
                    help="display cross-hairs target and depth from centre of image")
parser.add_argument("-hs", "--sidebysideh", action='store_true',
                    help="display left image and disparity side by side horizontally (stacked)")
parser.add_argument("-vs", "--sidebysidev", action='store_true',
...
parser.add_argument("-xml", "--config_file_xml", type=str, help="manual camera calibration XML configuration file",
                    default='')
parser.add_argument("-lrl", "--leftrightleft", action='store_true',
...
if (zed_cam.isOpened()):
    ...
    if (args.showcontrols):
        cv2.createTrackbar("Max Disparity(x 16): ", windowNameD, int(max_disparity / 16), 16,
        ...
        cv2.createTrackbar("Speckle Size: ", windowNameD, math.floor((width * height) * 0.0005), 10000,
                           on_trackbar_null)
        ...
        if (args.leftrightleft):
            ...
            cv2.createTrackbar("WLS Filter Sigma Color (x 0.1): ", windowNameD, math.ceil(wls_sigma / 0.1), 50,
    ...
    while (keep_processing):
        ...
        if (args.colourmap):
            disparity_to_display = cv2.applyColorMap((disparity_scaled * (256. / max_disparity)).astype(np.uint8),
                                                     cv2.COLORMAP_HOT)
        else:
