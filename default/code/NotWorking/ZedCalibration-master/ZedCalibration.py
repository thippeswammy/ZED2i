...
class ZED(object):
    ...
    def FindImagePoints(self):
        ...
        while (not (self.do_calibration)):
            ...
            if (self.fromFile == False):
                ...
                frameL, frameR = self.frameL[self.chessboard_pattern_detections], self.frameR[
                    self.chessboard_pattern_detections]
            ...
            retL, cornersL = cv2.findChessboardCorners(grayL, (self.patternX, self.patternY), None,
                                                       cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FAST_CHECK | cv2.CALIB_CB_NORMALIZE_IMAGE)
            retR, cornersR = cv2.findChessboardCorners(grayR, (self.patternX, self.patternY), None,
                                                       cv2.CALIB_CB_ADAPTIVE_THRESH | cv2.CALIB_CB_FAST_CHECK | cv2.CALIB_CB_NORMALIZE_IMAGE)
            ...
            key = cv2.waitKey(
                int(4000 / self.calibration_capture_framerate)) & 0xFF  # wait 500ms between frames - i.e. 2 fps
    ...
    def Calibrate(self, grayL, grayR):
        if (self.chessboard_pattern_detections >= self.maxImageCount):  # i.e. if we did not load a calibration
            ...
            ret, self.mtxL, self.distL, self.rvecsL, self.tvecsL = cv2.calibrateCamera(self.objpoints, self.imgpointsL,
                                                                                       (w, h), None, None)
            ret, self.mtxR, self.distR, self.rvecsR, self.tvecsR = cv2.calibrateCamera(self.objpoints, self.imgpointsR,
                                                                                       (w, h), None, None)
            ...
            while (self.keep_processing):
                ...
                key = cv2.waitKey(
                    int(1000 / self.disparity_processing_framerate)) & 0xFF  # wait 40ms (i.e. 1000ms / 25 fps = 40 ms)
    ...
    def CalculateReprojectionError(self):
        if (self.chessboard_pattern_detections >= self.maxImageCount):  # i.e. if we did not load a calibration
            ...
            for i in range(len(self.objpoints)):
                imgpointsL2, _ = cv2.projectPoints(self.objpoints[i], self.rvecsL[i], self.tvecsL[i], self.mtxL,
                                                   self.distL)
            ...
            tot_errorR = 0
            for i in range(len(self.objpoints)):
                imgpointsR2, _ = cv2.projectPoints(self.objpoints[i], self.rvecsR[i], self.tvecsR[i], self.mtxR,
                                                   self.distR)
    ...
    def ExtrinsicCalibration(self):
        ...
        if (self.chessboard_pattern_detections > 0):  # i.e. if we did not load a calibration
            ...
            h, w = self.imageShape
            (
            self.rms_stereo, self.camera_matrix_l, self.dist_coeffs_l, self.camera_matrix_r, self.dist_coeffs_r, self.R,
            self.T, self.E, self.F) = \
                cv2.stereoCalibrate(self.objpoints, self.imgpointsL, self.imgpointsR, self.mtxL, self.distL, self.mtxR,
                                    self.distR, (w, h), criteria=termination_criteria_extrinsics, flags=0)
    ...
    def RectifyImages(self):
        ...
        if (self.chessboard_pattern_detections > 0):  # i.e. if we did not load a calibration
            self.mapL1, self.mapL2 = cv2.initUndistortRectifyMap(self.camera_matrix_l, self.dist_coeffs_l, self.RL,
                                                                 self.PL, (1280, 720), cv2.CV_32FC1)
            ...
            self.mapR1, self.mapR2 = cv2.initUndistortRectifyMap(self.camera_matrix_r, self.dist_coeffs_r, self.RR,
                                                                 self.PR, (1280, 720), cv2.CV_32FC1)
            outputfile = "input/ZEDCalibration.npz"
    ...
    def CalculateStereoDepth(self):
        ...
        while (keep_processing):
            ...
            filteredImg = wls_filter.filter(displ, undistorted_rectifiedL, None,
            ...
            if (apply_colourmap):
                ...
                disparity_colour_mapped = cv2.applyColorMap((filteredImg * (256. / 160)).astype(np.uint8),
